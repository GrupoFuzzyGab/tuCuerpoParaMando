...
void setup() {
  ...
  kinect.enableDepth();
  kinect.enableRGB(); //visualizar color SYLVIA
}
void draw() {
  background(0);
  kinect.update();
  ...
  translate(0, 0, s*-1000); 
    ...
    scale(s);
  println(s);
   PImage rgbImage = kinect.rgbImage(); //VISUALIZAR COLOR SYLVIA
  ...
  for (int i = 0; i < depthPoints.length; i+=10) {
     stroke(rgbImage.pixels[i]); // PARA DAR COLOR SUSTITUIMOS stroke(255);
